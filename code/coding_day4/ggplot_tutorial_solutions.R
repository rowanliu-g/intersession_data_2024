#### SETUP ####
getwd()
setwd('/Users/17739/intersession_data_2024/code/coding_day4/')
getwd()
library(datasets)
library(viridis)
...
plt #allows you to run var on its own without running the whole line
=======
  labs(title = "Vehicle mpg and drive", x= "mpg in city", y= "mpg on highway" )

plt
>>>>>>> 75d52156181f101526528e5b0c73c2acf842dab3:code/coding_day4/ggplot_tutorial_solutions.R
...
<<<<<<< HEAD:code/coding_day4/ggplot_tutorial_inclass.R
  geom_histogram(mapping = aes(x=cty),
                               breaks = seq(from=0, to=35, by=5)) #doesn't run plot, it stores the plot

plt2 #actually runs the plot
...
plt3 <- ggplot(data = mpg) + 
  ...
  scale_color_viridis() +
  labs(title = "Vehicle pmg and engine type", x = "mpg in city", y = "mpg on highway", color = "# of cylinders") +
  theme_classic()
=======
plt3 <- ggplot(data = mpg) +
  geom_point(mapping = aes(x=cty, y=hwy, color = cyl)) +
